...
void Image::flipVertically()
{
	for (int y = 0; y < height / 2; y++)
	{
		...
		{
			Color c = colorMap[x + y * width];
			colorMap[x + y * width] = colorMap[x + (height - 1 - y) * width];
			colorMap[x + (height - 1 - y) * width] = c;
		}
	}
}
...
void Image::readDataAscii(std::ifstream &file, Image &image)
{
...
void Image::readDataRaw(std::ifstream &file, Image &image)
{
...
void Image::writeDataAscii(std::ofstream &file, Image &image)
{
	for (int y = 0; y < image.height; y++)
	{
		for (int x = 0; x < image.width; x++)
...
void Image::writeDataRaw(std::ofstream &file, Image &image)
{
